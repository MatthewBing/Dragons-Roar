...
public enum TextNumber {
    ...
    public static int lookupValue(String c) throws Exception { //make better variable name
        c = c.trim();
        for (TextNumber val : TextNumber.values()){
            if (val.getCode().equals(c)){
                return val.getValue();
            }
        }
        throw new DictionaryException(c);
    }

    //This will not work as expected, do it over!
    public static String lookupCode(int v) throws Exception{

        for (TextNumber strVal : TextNumber.values()){
            if (strVal.getValue() == v){
                return strVal.getCode();
            }
        }

        throw new DictionaryException(Integer.toString(v)); //do better
    }
